<script lang="ts">
	...
	let input: Input;
	$: firstSelectedCell =
		$highlights.selectedCells.length === 1 ? $highlights.selectedCells[0] : undefined;

	$: if (firstSelectedCell) {
		populateFromSelectedCell(firstSelectedCell);
	}

	$: $highlights.selectedCells,
		input != null && setTimeout(() => input != null && input.focus(), 100);
...
<div class="w-full h-full flex flex-col justify-center items-center p-2">
	<Input
		...
		title={$highlights.selectedCells.length !== 1
