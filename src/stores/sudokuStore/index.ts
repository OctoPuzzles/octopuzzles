...
function createEditorHistoryStore() {
	...
	const solution = writable<string[][] | null>(null);
...
function createGameHistoryStore() {
	...
	function verify(): void {
		...
		if (verificationMode === 'OnInput') {
			highlights.wrongCells.set(scanner.getErrorCells(get(editorHistory.solution)));
		} else {
			highlights.wrongCells.set([]);
		}
	}
...
export function setMargins(margins?: Margins | null): void {
	...
	const userInputs = get(gameHistory.subscribeToInputs());
	...
	if (offsets.left !== 0 || offsets.right !== 0 || offsets.top !== 0 || offsets.bottom !== 0) {
		...
		gameHistory.set({
			cellValues: offsetMatrix(userInputs.cellValues, offsets, {}),
			annotations: userInputs.annotations
				.map((annotation) => {
